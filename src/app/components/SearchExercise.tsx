...
export default function SearchExercise({setUrlMuscle, setUrlEquipment, fetchResults}:Props) {
    ...
    return (
        <div id="menu-container" className={`grid grid-flow-row grid-cols-2 tracking-wider text-xl p-8 font-d justify-stretch ${dela.className}`}>
            <div id="muscle" className="p-4 hover:bg-slate-500 cursor-pointer auto-rows-max auto-cols-max min-h-0  " onClick={handleClickM}>Muscle</div>

            <div id="equipment" className=" p-4 hover:bg-slate-500 cursor-pointer min-h-0 " onClick={handleClickE}>Equipment
            </div>
